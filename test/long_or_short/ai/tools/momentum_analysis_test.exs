defmodule LongOrShort.AI.Tools.MomentumAnalysisTest do
  use ExUnit.Case, async: true

  doctest LongOrShort.AI.Tools.MomentumAnalysis

  alias LongOrShort.AI.Tools.MomentumAnalysis

  describe "spec/0" do
    test "matches the Provider.tool_spec shape" do
      spec = MomentumAnalysis.spec()
    ...
    test "names the tool record_momentum_analysis" do
      assert MomentumAnalysis.spec().name == "record_momentum_analysis"
    end

    test "input_schema is an object with all expected properties" do
      schema = MomentumAnalysis.spec().input_schema
    ...
    test "does NOT include pump_fade_risk or strategy_match (Phase 1 stubs)" do
      props = MomentumAnalysis.spec().input_schema.properties
    ...
    test "marks core fields required, repetition_summary optional" do
      required = MomentumAnalysis.spec().input_schema.required
    ...
    test "catalyst_strength enum matches resource constraint" do
      enum = MomentumAnalysis.spec().input_schema.properties.catalyst_strength.enum
      assert Enum.sort(enum) == ~w(medium strong unknown weak)
    end

    test "catalyst_type enum matches resource constraint" do
      enum = MomentumAnalysis.spec().input_schema.properties.catalyst_type.enum
    ...
    test "sentiment enum matches resource constraint" do
      enum = MomentumAnalysis.spec().input_schema.properties.sentiment.enum
      assert Enum.sort(enum) == ~w(negative neutral positive)
    end

    test "verdict enum matches resource constraint" do
      enum = MomentumAnalysis.spec().input_schema.properties.verdict.enum
      assert Enum.sort(enum) == ~w(skip trade watch)
    end

    test "repetition_count requires minimum 1" do
      assert MomentumAnalysis.spec().input_schema.properties.repetition_count.minimum == 1
    end
  end
end
